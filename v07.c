    ...
    for (int i = 0; i < client->shopping_count; i++) {
        if (strcmp(client->shopping_list[i].name, good->name) == 0) {
            if (client->shopping_list[i].entity <= good->entity) {
                ...
                good->entity = client->shopping_list[i].entity; // Set the quantity the client wants to bu
...
int main() {
    ...
    for (int i = 0; i < client_count; i++) {
        ...
        if (client_pid == 0) {
            ...
            char *shops[] = {"/home/faramarzi/OSdataset/Store1", "/home/faramarzi/OSdataset/Store2", "/home/faramarzi/OSdataset/Store3"};
